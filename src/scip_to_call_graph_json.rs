...
pub fn build_call_graph(scip_data: &ScipIndex) -> HashMap<String, FunctionNode> {
    ...
    // First pass: identify all function symbols from symbol metadata
    ...
    // Second pass: find the definition occurrence (symbol_roles: 1) for each function symbol
    ...
    for doc in &scip_data.documents {
        ...
        for occurrence in &doc.occurrences {
            // Check if this is a definition occurrence (symbol_roles & 1 == 1)
            let is_definition = occurrence.symbol_roles.unwrap_or(0) & 1 == 1;

            if is_definition && function_symbols.contains(&occurrence.symbol) {
                // Only update if we haven't seen this symbol's definition before
                // This ensures we use the first definition we encounter
                if !call_graph.contains_key(&occurrence.symbol) {
                    ...
                    // Initialize node in the call graph with the correct file path
                    call_graph.insert(
                    ...
                    symbol_to_file.insert(occurrence.symbol.clone(), abs_path.clone());
                }
            }
        }
    }

    // Third pass: analyze occurrences to build the call graph (caller-callee relationships)
    for doc in &scip_data.documents {
        ...
        for occurrence in &ordered_occurrences {
            ...
            if is_definition && function_symbols.contains(&occurrence.symbol) {
                ...
                // Range is already set in the second pass, no need to update
            }
    ...
    // Fourth pass: extract function bodies from source files
    for node in call_graph.values_mut() {
        if node.range.is_empty() {
            eprintln!("WARNING: Function '{}' (symbol: {}) has no range information. Body extraction skipped.", 
                      node.display_name, node.symbol);
            node.body = Some(String::new());
            continue;
        }
            ...
            Ok(contents) => {
                ...
                if !node.range.is_empty() {
                    ...
                    if start_line < lines.len() {
                        ...
                        eprintln!("WARNING: Function '{}' (symbol: {}) has start_line {} out of bounds (file has {} lines). Body set to empty.", 
                                  node.display_name, node.symbol, start_line, lines.len());
                        node.body = Some(String::new());
                    }
                } else {
                    eprintln!("WARNING: Function '{}' (symbol: {}) has invalid range format (length < 1). Body set to empty.", 
                              node.display_name, node.symbol);
                    node.body = Some(String::new());
                }
            }
            Err(e) => {
                eprintln!("WARNING: Failed to read file '{}' for function '{}' (symbol: {}): {}. Body set to empty.", 
                          clean_path, node.display_name, node.symbol, e);
                node.body = Some(String::new());
            }
        }
    }
...
mod tests {
    ...
    fn test_function_body_extraction() {
        ...
        let expected_tooltip = "fn foo() { println!(\"Hello\"); }"
            .replace(['\n', '\r'], " ")
            .replace('"', "' ");
        assert!(dot.contains(&format!("tooltip=\"{expected_tooltip}\"")));
    }
}
