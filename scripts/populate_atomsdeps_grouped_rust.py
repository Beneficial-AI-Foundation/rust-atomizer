...
class PopulateAtomsDeps:
    ...
    def read_atomizer_logs(self, repo_id):
        ...
        try:
            ...
            if not log_files:
                self.logger.warning(
                    f"No atomizer log files found for repo_id {repo_id}"
                )
                return ""
            ...
            self.logger.info(
                f"Successfully read {len(atomizer_content)} characters from atomizer log"
            )
    ...
    def insert_captured_logs_to_db(self, repo_id, user_id):
        ...
        try:
            ...
            self.logger.info(
                f"Inserted combined logs to atomizerlogs table for repo_id {repo_id}"
            )
    ...
    def get_codes_ids(self, repo_id):
        ...
        folder_ids = set(
            row["folder_id"] for row in result if row.get("folder_id") is not None
        )
        ...
        if folder_ids:
            ...
            folder_id_to_info = {
                row["id"]: {"name": row["name"], "parent_id": row["parent_id"]}
                for row in all_folder_results
            }
        ...
        for row in result:
            ...
            self.logger.info(
                f"!!!!!Code ID {row['id']}: filename='{prefixed_filename}', folder_id={folder_id}, folder_path='{folder_path if folder_id else 'None'}'"
            )

        self.logger.info(f"Retrieved {len(codes_dict)} codes from database")
        return codes_dict

    def populate_atoms_from_json(
        self, repo_id, code_id, user_id, json_data, files_ids_dict
    ):
        try:
            ...
            if isinstance(data, list):
                ...
                raise ValueError(
                    "Invalid JSON format: neither an array nor contains 'Atoms' key"
                )
            ...
            if new_atoms:
                self.populate_atoms_table_batch(
                    repo_id, code_id, user_id, new_atoms, files_ids_dict
                )

        except Exception as e:
            error_msg = (
                f"Error in populate_atoms_from_json: {str(e)}\n{traceback.format_exc()}"
            )
            self.logger.error(f"{error_msg} - repo_id={repo_id}, code_id={code_id}")

    def populate_atoms_table_batch(
        self, repo_id, code_id, user_id, atoms_list, files_ids_dict
    ):
        """
        ...
        for file_path, file_atoms in atoms_by_file.items():
            ...
            if file_path in files_ids_dict:
                ...
                if result:
                    ...
                    if result:
                        ...
                        self.logger.warning(
                            f"Failed to get molecule ID for {file_path}"
                        )
                        continue
            ...
            if values:
                ...
                self.logger.info(
                    f"Inserted {len(file_atoms)} atoms for file {file_path}"
                )
            else:
        ...
        if not valid_deps:
            ...
            for parent_identifier, child_identifier in dependency_pairs:
                if parent_identifier not in id_map:
                    missing.append(
                        f"Missing parent: {parent_identifier} for child {child_identifier}"
                    )
                if child_identifier not in id_map:
                    missing.append(
                        f"Missing child: {child_identifier} for parent {parent_identifier}"
                    )
            if missing:
    ...
    def filter_json_by_filename(self, json_content, filename):
        ...
        try:
            ...
            # Handle the case where the JSON has an "Atoms" key
                ...
                raise ValueError(
                    "Invalid JSON structure: neither an array nor contains 'Atoms' key"
                )
    ...
    def build_folders_to_files_mapping(self, json_path):
        ...
        self.logger.info(
            f"Identified {len(folders_to_files)} folders containing Rust files"
        )
    ...
    def populate_all_atoms_for_rust(self, repo_id, json_path, files_ids_dict):
        ...
        try:
            if codes_cache_path.exists():
                ...
                codes = {
                    code_id: code_data
                    for code_id, code_data in self.get_codes_ids(repo_id).items()
                ...
                for code_data in codes.values():
                    if isinstance(code_data["text"], bytes):
                        code_data["text"] = code_data["text"].decode(
        ...
        self.logger.debug(
            f"Found {len([atom for atom in atoms_list if atom.get('relative_path', '') == 'sm2/src/lib.rs'])} atoms for sm2/src/lib.rs in JSON"
        )
        ...
        for code_id, code_data in codes.items():
            ...
            for atom in atoms_list:
                ...
                if relative_path == filename:
                    filtered_atoms.append(atom)
                    self.logger.debug(
                        f"Matched atom {identifier} to filename {filename} via exact path match"
                    )
            ...
            if filtered_atoms:
                ...
                for atom in atoms_list[:100]:  # Check first 100 atoms to avoid spam
                    rp = atom.get("relative_path", "")
                    if filename_base in rp or any(
                        part in rp for part in filename.split("/")[-2:]
                    ):
                        similar_paths.append(rp)

                if similar_paths:
                    unique_similar = list(set(similar_paths))[
                        :5
                    ]  # Show up to 5 unique similar paths
                    self.logger.debug(f"Similar paths found: {unique_similar}")

        # Debug: Show summary of filename_to_atoms mapping
        self.logger.info(
            f"Summary: Found atoms for {len(filename_to_atoms)} out of {len(codes)} files"
        )
        ...
        for code_id, code_data in codes.items():
            ...
            if not new_atoms:
                self.logger.debug(
                    f"No new atoms to process for {filename} and code_id {code_id}"
                )
                continue
            ...
            self.populate_atoms_from_json(
                repo_id, code_id, user_id, json_for_filename, files_ids_dict
            )
        ...
        for code_id, code_data in codes.items():
            ...
            if not atoms_for_file:
                self.logger.debug(
                    f"No atoms found for {filename} and code_id {code_id}"
                )
                continue
    ...
    def populate_folder_structure_as_molecules(
        self, user_id, repo_id, folders_to_files
    ):
        """
        ...
        """
        self.logger.info(
            f"Populating folder structure with {len(folders_to_files)} folders as molecules for repo {repo_id}"
        )
        ...
        self.logger.info(
            f"All folders to create (including parents): {all_sorted_folders}"
        )
        ...
        for folder_path in all_sorted_folders:
            ...
            if result:
                ...
                self.logger.debug(
                    f"Added folder {folder_name} (full_identifier: {folder_full_identifier}) with parent_id {parent_id}"
                )
        ...
        for folder_path, files in folders_to_files.items():
            folder_full_identifier = (
                folder_path.rstrip("/") if folder_path != "/" else "/"
            )
        ...
        new_files = [
            (file_identifier, folder_id)
            for file_identifier, folder_id in all_file_data
        ...
        self.logger.info(
            f"Successfully populated {len(identifier_to_id)} folder and file molecules for repo {repo_id}"
        )
    ...
    def set_code_id_for_files_old(self, repo_id):
        ...
        self.logger.info(
            f"Found {len(files_needing_update)} file molecules with code_id=0 to update"
        )
        self.logger.info(
            f"Found {len(files_with_code_id)} file molecules with specific code_ids"
        )
        ...
        for file_id in file_ids:
            ...
            file_info = next(
                (row for row in files_needing_update if row["id"] == file_id), None
            )
            file_identifier = (
                file_info["full_identifier"] if file_info else f"ID_{file_id}"
            )
            ...
            if atom_result:
                ...
                self.logger.warning(
                    f"No child atoms found for file ID {file_id} (file: {file_identifier})"
                )
                ...
                """
                count_result = sql2(
                    self.con, specific_file_query, (file_identifier, repo_id)
                )
                if count_result and count_result[0]["count"] > 0:
                    self.logger.info(
                        f"  ℹ️  Found {count_result[0]['count']} atoms for {file_identifier} in files with specific code_ids"
                    )
        ...
        self.logger.info(
            f"Successfully updated code_id for {len(file_id_to_code_id)} file molecules"
        )
...
if __name__ == "__main__":
    if len(sys.argv) not in [3, 4]:
        print(
            "Usage: python populate_atomsdeps_table_rust.py <repo_id> <json_path> [user_id]"
        )
        sys.exit(1)
    ...
    populate_atoms_deps = PopulateAtomsDeps(
        con, log_to_file=True, log_filename=log_filename, log_level=log_level
    )

    # Log the input parameters and JSON file being processed
    populate_atoms_deps.logger.info(
        "Starting populate_atomsdeps_grouped_rust.py with parameters:"
    )
    ...
    if json_path.exists():
        populate_atoms_deps.logger.info(
            f"  JSON file size: {json_path.stat().st_size} bytes"
        )
        populate_atoms_deps.logger.info(
            f"  JSON file modified: {datetime.fromtimestamp(json_path.stat().st_mtime)}"
        )
    ...
    user_id = 460176
    files_ids_dict = populate_atoms_deps.populate_folder_structure_as_molecules(
        user_id, repo_id, folders_to_files
    )
    ...
    # Log completion
    populate_atoms_deps.logger.info(
        f"Successfully completed populate_atomsdeps_grouped_rust.py for repo_id {repo_id}"
    )
