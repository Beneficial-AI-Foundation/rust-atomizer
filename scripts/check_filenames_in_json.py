        ...
        with open(log_file_path, "r") as f:
            for line_num, line in enumerate(f, 1):
                ...
                # Alternative: Look for lines mentioning JSON file being processed
                    ...
                    match = re.search(
                        r"Processing completed for JSON file:\s*(.+)$", line.strip()
                    )
                    if match:
                        json_path = match.group(1).strip()
                        json_filename = os.path.basename(json_path)
        ...
        with open(log_file_path, "r") as f:
            for line_num, line in enumerate(f, 1):
                if "No atoms found for filename:" in line:
                    ...
                    match = re.search(
                        r"No atoms found for filename:\s*(.+)$", line.strip()
                    )
                    if match:
...
def main():
    ...
    print(
        "Generated no_atoms_filenames.txt with {} unique entries".format(
            len(set(filenames_from_log))
        )
    )
    print("JSON analysis complete - see results above")
